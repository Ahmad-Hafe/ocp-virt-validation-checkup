...
if [ "${DS_LABEL}" == "ocp-virt-validation" ] && [ "${ACCEPT_WINDOWS_EULA}" != "true" ]; then
  ...
  NS_LABEL="$(oc get namespace "${GOLDEN_IMAGE_NAMESPACE}" -o jsonpath='{.metadata.labels.app}' 2>/dev/null || true)"
  if [ "${NS_LABEL}" == "ocp-virt-validation" ]; then
    oc delete namespace "${GOLDEN_IMAGE_NAMESPACE}" --wait=false 2>/dev/null || true
  else
    echo "Namespace ${GOLDEN_IMAGE_NAMESPACE} is not tool-managed, cleaning up individual labeled resources only"
    oc delete datasource -l app=ocp-virt-validation -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
    oc delete pvc -l app=ocp-virt-validation -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
    oc delete pipelinerun -l app=ocp-virt-validation -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
    oc delete configmap -l app=ocp-virt-validation -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
    oc delete rolebinding -l app=ocp-virt-validation -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
    oc delete role -l app=ocp-virt-validation -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
    oc delete sa -l app=ocp-virt-validation -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
  fi
  oc adm policy remove-scc-from-user privileged -z pipeline -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
  oc delete clusterrolebinding validation-pipeline-privileged-scc 2>/dev/null || true
  DS_READY=""
fi
...
PYEOF
  WIN_IMAGE_FLAG="--tc-file=${WIN_LOGIN_CONFIG} --tc=win_golden_image_name:${GOLDEN_IMAGE_NAME} --tc=storage_class_a:${STORAGE_CLASS} --tc=storage_class_b:${STORAGE_CLASS}"
else
