...
cleanup_namespace() {
  ...
  if [ "${NS_LABEL}" == "ocp-virt-validation" ]; then
    echo "Cleaning up tool-managed namespace ${GOLDEN_IMAGE_NAMESPACE}..."
    oc adm policy remove-scc-from-user privileged -z pipeline -n "${GOLDEN_IMAGE_NAMESPACE}" 2>/dev/null || true
...
if [ "${DS_READY}" == "True" ]; then
  ...
  echo "Skipping golden image setup — using existing user-provided image."
  echo ""
  echo "=== Windows Server 2022 Golden Image Ready (BYOI) ==="
...
# If a tool-created PVC exists from a previous run, clean it up before re-running
if oc get pvc "${GOLDEN_IMAGE_NAME}" -n "${GOLDEN_IMAGE_NAMESPACE}" &>/dev/null; then
  ...
  if [ "${PVC_LABEL}" == "ocp-virt-validation" ]; then
    echo "PVC '${GOLDEN_IMAGE_NAME}' exists from a previous tool run -- cleaning up."
