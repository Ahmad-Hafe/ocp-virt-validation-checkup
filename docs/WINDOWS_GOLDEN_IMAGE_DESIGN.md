...
6. [Design Evolution & Key Decisions](#6-design-evolution--key-decisions)
7. [Final Architecture](#7-final-architecture)
8. [User Flows](#8-user-flows)
9. [Resource Lifecycle & Cleanup](#9-resource-lifecycle--cleanup)
10. [Obstacles & How They Were Resolved](#10-obstacles--how-they-were-resolved)
11. [Verification Matrix](#11-verification-matrix)
12. [Risks & Mitigations](#12-risks--mitigations)
13. [Future Work](#13-future-work)
14. [Related Work & Dependencies](#14-related-work--dependencies)
...
| **Priya Parasuram** | Program Manager | Cloud Provider Validation | Project tracking, sprint priorities, scope management. Runs the cloud provider sync meetings where this feature is tracked. |
...
|---|------|----------|--------|
| G1 | Standalone Windows golden image creation without Artifactory or internal registry dependency | P0 | :white_check_mark: Done (PR #87) |
| G2 | Windows hotplug tests (existing Tier-3) running in self-validation | P0 | :white_check_mark: Done (PR #87 + OVT #4571) |
| G3 | BYOI support — customer can bring their own Windows PVC via manifest | P0 | :white_check_mark: Done (PR #87) |
| G4 | Deterministic cleanup — tool never destroys user resources | P0 | :white_check_mark: Done (PR #87) |
| G5 | UI exposure of EULA acceptance checkbox | P1 | :hourglass: In progress ([CNV-88481](https://redhat.atlassian.net/browse/CNV-88481)) |
| G6 | Disconnected environment support | P1 | :white_check_mark: Documented |

### Non-Goals (Out of Scope for This PR)

| # | Non-Goal | Reason | Tracked By |
|---|----------|--------|------------|
| NG1 | Multi-disk concurrent boot test (20 VMs, 4 disks each) | Separate feature — different GA requirement | [CNV-88906](https://redhat.atlassian.net/browse/CNV-88906) |
| NG2 | Snapshot & restore 4 VMs in parallel | Separate feature — different GA requirement | [CNV-88908](https://redhat.atlassian.net/browse/CNV-88908) |
| NG3 | Multi-disk cloning / host-assisted cloning | Separate feature — different GA requirement | [CNV-88909](https://redhat.atlassian.net/browse/CNV-88909) |
| NG4 | Multi-disk live and storage migration | Separate feature — in review | [CNV-88910](https://redhat.atlassian.net/browse/CNV-88910) / [CNV-88911](https://redhat.atlassian.net/browse/CNV-88911) |
| NG5 | Windows app-consistent snapshot with timing | Requires additional Windows tooling beyond current scope | — |
| NG6 | Supporting Windows versions other than Server 2022 | Server 2022 aligns with existing Tier-3 tests; other versions deferred | — |
| NG7 | Backport to 4.22.z | Per Ramon Lobillo — test thoroughly in 5.0 first | — |
...
## 6. Design Evolution & Key Decisions
...
## 10. Obstacles & How They Were Resolved
...
|---|------|----------|------------|------------|
| R1 | **OpenShift Pipelines not installed** — EULA path requires Tekton to build the golden image from ISO | High | Medium | Documented as prerequisite. Fail-fast with clear error message. BYOI path has no Tekton dependency. |
| R2 | **Windows ISO download changes** — Microsoft may change download URLs or auth requirements | Medium | Medium | ISO URL is configurable via ConfigMap. Customer can update without code changes. |
| R3 | **Pipeline timeout on slow storage** — golden image build involves ~20 GB download + sysprep; some cloud storage classes (ANF, GCNV) have known slow provisioning | Medium | High | Pipeline timeout is configurable. Known issue documented for ANF with provisioning queuing. |
| R4 | **Scope creep into broader GA requirements** — Windows support is one of 11 GA storage criteria; pressure to bundle multi-disk, snapshot, and migration tests into this PR | High | High | Explicit non-goals defined (NG1–NG4). Each has its own Jira card and can be delivered independently. |
| R5 | **Backport regression** — if backported to 4.22.z without sufficient testing | Medium | Low | Non-goal for this PR (NG7). Ramon Lobillo's guidance: test thoroughly in 5.0 first. |
| R6 | **BYOI image incompatibility** — user-provided Windows image may not have required drivers (VirtIO, QEMU Guest Agent) | Medium | Medium | Documented prerequisites. Tests fail fast with clear assertion if image is not bootable or missing guest agent. |

---

## 13. Future Work

This section tracks work that builds on the Windows golden image foundation but is **out of scope for PR #87**.

### Sprint Priorities (as of Jul 13, 2026 sync with Natalie & Priya)

```mermaid
gantt
    title Self-Validation Suite — Upcoming Deliverables
    dateFormat  YYYY-MM-DD
    axisFormat  %b %d

    section P0 — Windows
    Windows self-validation (PR #87 + OVT #4571)    :done,    win,   2026-06-08, 2026-07-19
    CNV-89353 — T3 Windows in self-validation        :done,    t3w,   2026-06-01, 2026-07-13

    section P0 — Multi-disk
    CNV-88906 — 20 VMs, 4 disks concurrent boot      :active,  md1,   2026-07-14, 2026-07-25

    section P1 — Snapshot & Clone
    CNV-88908 — Snapshot & restore 4 VMs parallel     :         snap,  2026-07-14, 2026-07-25
    CNV-88909 — Multi-disk cloning                    :         clon,  2026-07-14, 2026-07-25

    section P2 — Migration
    CNV-88910/88911 — Live & storage migration        :         mig,   2026-07-21, 2026-08-01

    section P2 — Self Certification
    Define T1 subset for GA self-certification        :         cert,  2026-08-01, 2026-08-15
```

### Planned Items

| Jira | Description | Priority | Status |
|------|-------------|----------|--------|
| [CNV-88906](https://redhat.atlassian.net/browse/CNV-88906) | Multi-disk concurrent boot (20 VMs, 4 disks each) | P0 | Not Started |
| [CNV-88908](https://redhat.atlassian.net/browse/CNV-88908) | Snapshot & restore 4 VMs in parallel | P1 | Not Started |
| [CNV-88909](https://redhat.atlassian.net/browse/CNV-88909) | Multi-disk cloning / host-assisted cloning | P1 | Not Started |
| [CNV-88910](https://redhat.atlassian.net/browse/CNV-88910) / [CNV-88911](https://redhat.atlassian.net/browse/CNV-88911) | Multi-disk live and storage migration | P2 | In Progress (PR in review) |
| [CNV-88481](https://redhat.atlassian.net/browse/CNV-88481) | UI: Expose EULA acceptance checkbox | P1 | In Progress |
| — | Define T1 subset for GA self-certification suite | P2 | Not Started |

### Long-Term Vision: Self-Certification

The ultimate goal (as stated in the cloud provider sync meetings) is:

> *"We want to graduate this suite to become a self-certification suite, so that we don't have to run any tests ourselves."*

This means storage partners (Pure, NetApp/GCNV, GCP Hyperdisk, etc.) can independently validate their storage by running the self-validation suite — including Windows coverage — without Red Hat engineering involvement. PR #87 is a foundational step toward that goal, eliminating the last manual testing gap (Windows) from the conformance suite.

### Open Questions

| # | Question | Owner | Context |
|---|----------|-------|---------|
| Q1 | Should the BYOI manifest support importing from a container registry (in addition to PVC clone and HTTP)? | Felix / Karel | Currently only PVC clone and HTTP import are documented |
| Q2 | What is the upgrade path for golden images across OCP versions? | Ruth / Dominik | Image may need re-sysprep after major OCP upgrade |
| Q3 | Should Windows volume-expansion and snapshot tests also be added to self-validation? | Natalie / Ahmad | Part of Peter's GA criteria but separate Jira cards |

---

## 14. Related Work & Dependencies
...
- **Project Tracking (Priya):** [Cloud Provider Sync Meeting Notes](https://docs.google.com/document/d/146x40wpeLPCYVcVMOQjwYno29qvdlh3OpSPxorLlkj0/edit)
...
- **Windows Self-Validation Jira:** [CNV-89353](https://redhat.atlassian.net/browse/CNV-89353)
